#simulate genotype-phenotype data - UseSLIC data LD patterns (sample from larger SLIC Corr matrix and simulate Nsnps)
...
gsca_sim_multigene3<-function(nsnp,nsnpeff,ngenes=1,nsub=120,ncases=100000,gpcorr=0.4,n2sim=3,plotter=TRUE,cb=cb,cluster=cluster,phenCorr=phenCorr)
{
  ...
  if(plotter==TRUE){
    ...
    print(g1)
    ggsave(paste0("/Users/paulthompson/Dropbox/project SCT analysis/GSCA validation/July2019/Corr_plots/plot_SLIC_corr_random_pheno_Pattern_",cb,".pdf"))
  ...
  write.table(t(model_out_summary2), file = paste0("/Users/paulthompson/Dropbox/project SCT analysis/GSCA validation/July2019/simulations_results/SLIC_LD_pheno_random_pattern_negatives_01.csv"), sep = ",", append = TRUE, quote = FALSE, col.names = FALSE, row.names = FALSE)
...
combos<-expand.grid(nsnpeff=c(0,5,10),nsnp=c(20,40),ngenes=c(1),nsub=c(100),cluster=c(0),gpcorr=c(0.1,0.15,0.2),phenCorr=c(0,0.5,0.75))
...
write.csv(combos,"/Users/paulthompson/Dropbox/project SCT analysis/GSCA validation/July2019/simulations_results/SLICcombos.csv")
...
file.name <- "SLIC_LD_pheno_random_pattern_negatives_01.csv"
