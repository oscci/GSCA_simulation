...
myaddress<-pauladdress #switch if necessary for Paul
...
test_combos<-data.frame(combos_plink,BH=rep(NA,length(combos_plink[,1])),BY=rep(NA,length(combos_plink[,1])))
...
#Both methods scan the snps for any p-value below adjusted alpha
    ...
    SNP_pv<-as.vector(unlist(SNP_p))
    SNP_p_BH<-p.adjust(SNP_pv,method = "BH")
    SNP_p_BY<-p.adjust(SNP_pv,method = "BY")
    #
    vA<-length(which(SNP_p_BH<(myalpha))) 
    vB<-length(which(SNP_p_BY<(myalpha))) 
   if(vA>0){nA<-nA+1} #increase nA if any SNP has sig result
   if(vB>0){nB<-nB+1}
  }

  test_combos$BH[cb]<-nA/nrun
   test_combos$BY[cb]<-nB/nrun
  
}

writeaddress<-paste0(myaddress,'August2020/test_combos_regression_plink_rep_FDR',nrun,'.csv')
...
Can either plot results from Benjamini-Hochberg corrected, or for Benjamini-Yekutieli. I have manually altered which of these is selected in the my_summary_new function, and also altered the saved filenames for these 2 options.
...
my_summary_new<-function(bigsummary=test_combos)
{
  ...
  groupedsummary_new<-by_N_neff_nsub %>% summarise(power_BH = mean(BH), power_BY = mean(BY))
 ...
 groupedsummary_new$Nsnpeffplot<-groupedsummary_new$nsnpeff
  ...
  groupedsummary_new$power_BH<-100*groupedsummary_new$power_BH
  groupedsummary_new$power_BY<-100*groupedsummary_new$power_BY
...
groupedsummary_new2<-groupedsummary_new 
groupedsummary_new2 <- gather(groupedsummary_new,key="method",value="power",power_BH:power_BY)
groupedsummary_new2$method <- dplyr::recode(groupedsummary_new2$method, power_BH="FDR-BH",power_BY="FDR-BY")
# To use for line and point colors, add

png(paste0(myaddress,'August2020/DB_plots/PLINK_Random_nrun_FDR',nrun,'.png'))
ggplot(groupedsummary_new2,aes(x=Nsnpeffplot,y=power,colour=PhenoCorr,shape=nsub,linetype=method))+
  geom_point()+
  geom_line()+
...
mypath <- paste0(myaddress,"August2020/simulations_results/")
...
db_data<-read_csv(paste0(myaddress,'August2020/test_combos_regression_plink_rep_FDR1000.csv'))

db_data2<-db_data %>% 
  dplyr::filter(.,nsub==100)%>%
  dplyr::rename(power = BH) %>%
  mutate(method=rep("PLINK (FDR-BH)")) 
...
tiff(paste0(myaddress,'August2020/DB_plots/PLINK_GSCA_comparison_plot_FDR.tiff'),width=2400,height=2000,res = 400)
