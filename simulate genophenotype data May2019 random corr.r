...
gsca_sim_multigene3<-function(nsnp,nsnpeff,ngenes=1,nsub=120,ncases=100000,gpcorr=0.4,n2sim=3,plotter=TRUE,cb=cb,cluster=cluster)
{
  ...
  # MyRandCorrMat2<-function(h=nsnp)
  # {
  # R <- matrix(rbeta(h^2,2,4), ncol=h) 
  # RtR <- R %*% t(R) 
  # Q <- cov2cor(RtR)
  # return(Q)
  # }
  ...
  if(plotter==TRUE){
    ...
    g1<-ggplot(data = cor_tri_N, aes(Var2, Var1, fill = value)) + 
      geom_tile()+scale_fill_gradient(low = "yellow", high = "red")+theme_bw()+ theme(axis.text.x = element_text(angle = 90, hjust = 1),panel.grid.major = element_blank(), panel.grid.minor = element_blank(),axis.title = element_blank(),legend.title=element_blank(),text=element_text(size=14))
    
    print(g1)
    ggsave(paste0("/Users/paulthompson/Dropbox/project SCT analysis/GSCA validation/May 2019/simulations_results/plots_random_corr_mat/plot_rand_corr_",cb,".pdf"))
  ...
  write.table(myfitsummaryPT, paste0("/Users/paulthompson/Dropbox/project SCT analysis/GSCA validation/May 2019/simulations_results/simulation_rand_corr_",cb,"_results.csv"), sep=",",row.names=FALSE) 
  ...
  model_out_summary2<-c(nsnp,nsnpeff,ngenes,combos[cb,6],gene_power2,nrun,combos[cb,5],combos[cb,4])
  
  
  write.table(t(model_out_summary2), file = paste0("/Users/paulthompson/Dropbox/project SCT analysis/GSCA validation/May 2019/simulations_results/PT_simulation_results_May2019_rand_corr.csv"), sep = ",", append = TRUE, quote = FALSE, col.names = FALSE, row.names = FALSE)
...
combos<-expand.grid(nsnpeff=c(0,5,10),nsnp=c(20,40),ngenes=c(1),nsub=c(100),cluster=c(0),gpcorr=c(0.1,0.4))

for(cb in 1:length(combos[,1]))
{
  print(paste0("combo",cb))
  gsca_sim_multigene3(nsnp=combos[cb,2],nsnpeff=combos[cb,1],ngenes=combos[cb,3],nsub=combos[cb,4],ncases=10000,gpcorr=combos[cb,6],n2sim=3,plotter = TRUE,cb=cb,cluster=combos[cb,5])
