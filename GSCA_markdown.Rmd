...
mynperm=1000 #for GSCA - probably need more than this but to test prog use 100 for speed
dogsca<-1
npop<-10000 #size of population to simulate
n2sim<-3 #n phenos indexing latent pheno factor : nb phenotypes correlate with latent factor

LDbase <- 0 #This is an index that controls the correlation between adjacent SNPs - correl will decline with distance
...
effsizelist<-c(1) #will be divided by 10 to give a correlation
...
nsnpefflist<-c(5,10) #N of SNPs with an effect on phenotype (NB in some versions of script we specify a proportion - but in this version we use N. Note that for a given nsnp, proportion will decrease as N SNPs increases, e.g. 10/100 SNPs is only 10% with an effect, where 10/20 SNPs is 50%).
...
nsublist<-c(100) #number of cases in the simulation - simulation will cycle through all values listed here

nsnp_per_gene<-c(20) #N SNPs that are tested - only a subset of these will have an effect on the phenotype - defined by nsnpefflist. If just one gene, then these correspond to all SNPs in the simulation.
...
makedata <- function(nsnp,ngenes,effsize,nsnpeff,LDbase){
    ...
    #We have +4 to allow for additional columns for (a)latent phenotype and (b) 3 component variables
    ...
    #values for phenotype now added
    #created as correlated with latent: r = phencorr
    #formula creates correlated variables as weighted sum of latent and random error
...
ggplot(groupedsummary_new,aes(x=nsnpeffplot,y=power,colour=ngenes,shape=nsub))+geom_point()+geom_line(aes(linetype=gene))+facet_grid(nsnp~effsize)+theme_bw()+theme(legend.position = "bottom")
...
```{r paulcompare}
#Reading previously computed data and selecting for comparison
  paultable<-read_csv('PT_simulation_results_MC_adj_formatted-1.csv')
paulcompare<-filter(paultable,ngenes==1,cluster==0,nsnp<100)
paulcompare<-dplyr::select(paulcompare,-c(nsig2,nsig3,nsig4,cluster))
paulcompare<-dfOrder(paulcompare,c(7,1,2,4))
write_csv(paulcompare,'paulcompare.csv')
```
...
dolm<-function(myread,myruns,myN){
  ...
  for (r in 1:myruns){
    ...
    #For each phenotype measure, we are just going to search for the largest correlation with a SNP and test that for significance
          ...
          for (j in (nsnp+2):(nsnp+4)){ #skip latent phenotype
            ...
            if (myp<mybonfp){
              myposresults[r,(j-nsnp-1)]<-1 #myposresults will be matrix
...
plot((100-bigtable2$gscanull)~bigtable2$meanES,ylim=c(0,100),col=bigtable2$Ncolor,pch=bigtable2$snpshape,xlab='Mean Effect size',ylab='% significant runs',main='GSCA')
legend(.1, 55, legend=c("N=100", "N=500"),
       col=c(1,2), lty=c(1,1), cex=0.8)
legend(.1, 35, legend=c("nsnp=20", "nsnp=40","nsnp=100"),
      pch=c(15,16,17), cex=0.8)
...
#To see how meanES relates to %significant, reorder bigtable2
newdata <- bigtable2[order(bigtable2$Npats,bigtable2$meanES),]
